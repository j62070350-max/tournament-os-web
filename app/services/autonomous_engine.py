...
async def _run_once() -> None:
    ...
    async with AsyncSessionLocal() as session:
        # Scan ALL tournaments that aren't ARCHIVED or CANCELLED and have autonomous_mode = True
        q = (
            select(Tournament)
            .where(
...
async def _process_tournament(tournament_id: str, organization_id: str) -> None:
    ...
    async with AsyncSessionLocal() as session:
        ...
        # --- PHASE 0: AUTO-REGISTRATION ---
        if t.status == TournamentStatus.DRAFT and t.registration_open_at and now >= t.registration_open_at:
            logger.info("Autonomous: Opening registration for tournament %s", tournament_id[:8])
            async with session.begin():
                t.status = TournamentStatus.REGISTRATION_OPEN
                # Trigger registration panel posting
                asyncio.create_task(_post_registration_panel(tournament_id, organization_id))
            return

        # --- PHASE 1: AUTO-CHECKIN ---
        if t.status == TournamentStatus.REGISTRATION_OPEN and t.registration_close_at and now >= t.registration_close_at:
            logger.info("Autonomous: Closing registration, opening check-in for tournament %s", tournament_id[:8])
            async with session.begin():
                t.status = TournamentStatus.CHECKIN_OPEN
            return

        # --- PHASE 2: AUTO-START (REG -> LIVE) ---
        if t.status == TournamentStatus.CHECKIN_OPEN and t.checkin_close_at and now >= t.checkin_close_at:
            ...
            # 2. Generate Bracket and create match channels
            # We do this outside the session.begin() to avoid long transactions
            asyncio.create_task(_auto_generate_first_round(tournament_id, organization_id))
            return

        if t.status != TournamentStatus.LIVE:
            return

        # --- PHASE 3: LIVE PROGRESSION (Existing Logic) ---
        ...
        if not all_matches:
            # If LIVE but no matches, we might have failed to generate. Try again.
            asyncio.create_task(_auto_generate_first_round(tournament_id, organization_id))
            return
